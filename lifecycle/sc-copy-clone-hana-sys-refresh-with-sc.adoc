...
|  | Cifrado persistente de SAP HANA | SFSS: inicialmente, o si se han cambiado las claves raíz en el sistema de origen, las copias de seguridad de las claves raíz deben importarse en el sistema de destino antes de que se pueda ejecutar la recuperación. LSS: inicialmente, o si se ha cambiado la contraseña de la copia de seguridad de la clave raíz, la contraseña debe establecerse en la configuración de LSS. 
...
|  | Cifrado persistente de SAP HANA | SFSS: Si un DBID del sistema de origen no existe antes en el sistema de destino, la base de datos del sistema debe recuperarse primero, antes de que se pueda importar la copia de seguridad de la clave raíz de este tenant. LSS: La topología LSS se sincronizará automáticamente con la topología HANA durante la recuperación de la(s) base(s) de datos del tenant. 
...
Todos los sistemas SAP HANA deben configurarse según la guía de configuración link:../bp/hana-aff-nfs-introduction.html["SAP HANA en sistemas AFF de NetApp con NFS"]. SnapCenter y los recursos de SAP HANA se configuraron según la guía de mejores prácticas link:../backup/hana-sc-generic-scope.html["Backup y recuperación de datos de SAP HANA con SnapCenter"].
...
. Configuración del sistema SAP HANA en SnapCenter como se describe en link:../backup/hana-sc-generic-scope.html["Backup y recuperación de datos de SAP HANA con SnapCenter"]
+
...
En una configuración SAN Fibre Channel, necesitas indicar el protocolo utilizado en la siguiente pantalla.
