...
Se creó un usuario de base de datos “SnapCenter” para la base de datos del sistema HANA con los privilegios necesarios para ejecutar operaciones de respaldo y recuperación (consulte link:hana-sc-generic-scope.html["Backup y recuperación de datos de SAP HANA con SnapCenter"^]). Se debe configurar una clave de almacén de usuarios de HANA en ambos hosts utilizando el usuario de base de datos mencionado anteriormente.
...
Las operaciones de restauración y recuperación para una configuración de grupo de recursos de SnapCenter 4.6 son idénticas a las de una configuración de replicación no del sistema descubierta automáticamente. Están disponibles todas las opciones de restauración y recuperación automática. Para más detalles, consulte el informe técnico. link:hana-sc-generic-scope.html["Backup y recuperación de datos de SAP HANA con SnapCenter"^].
