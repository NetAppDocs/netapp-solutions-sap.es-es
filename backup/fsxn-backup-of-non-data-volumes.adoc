...
+
El escenario estándar para los sistemas de inquilino único de HANA MDC es utilizar recursos completos con reversión de volumen. Para un sistema HANA MDC con múltiples inquilinos, es posible que desee restaurar solo un inquilino. Para obtener más información sobre la restauración de un solo inquilino, consulte link:hana-sc-generic-restore.html["Restauración y recuperación (netapp.com)"^].
