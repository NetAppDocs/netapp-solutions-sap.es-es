...
NOTE: Los ejemplos muestran una base de datos SAP HANA con SID=NF2 usando LSS.

Para crear los subdirectorios necesarios, realice una de las siguientes acciones:

* Para un sistema de un solo host, monta el volumen `/hana/shared` y crea los subdirectorios `shared`, `lss-shared` y `usr-sap`.
...
....
* Para un sistema con múltiples hosts, monta el `/hana/shared` volumen y crea los subdirectorios `shared`, `lss-shared` y el `usr-sap` para cada host.
+
...
| /lss/shared | NFSv3 o NFSv4 | NFSv3 o NFSv4 
...
....
. Ejecuta `mount -a` para montar los sistemas de archivos en todos los hosts.


El siguiente ejemplo muestra una base de datos SAP HANA de múltiples hosts con SID=NF2 usando NFSv4.1 para los sistemas de archivos de datos y de registro y NFSv3 para los sistemas de archivos  `/hana/shared`, `/lss/shared` y  `/usr/sap/NF2`. Se usa un tamaño de transferencia NFS de 1MB para lecturas y 256k para escrituras.
