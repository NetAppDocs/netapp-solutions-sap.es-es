...
Las instantáneas creadas por ONTAP Autonomous Ransomware Protection no se pueden usar para la recuperación anticipada de la base de datos SAP HANA. Por eso, no se recomienda usarlas para los volúmenes de datos y registros de SAP HANA y debería desactivarse para estos volúmenes. Usa la protección de datos basada en SnapCenter con instantáneas a prueba de manipulaciones para protegerte contra ransomware. link:../backup/hana-sc-generic-scope.html["Backup y recuperación de datos de SAP HANA con SnapCenter"] ONTAP Autonomous Ransomware Protection puede ser útil para otros volúmenes como /hana/shared.
