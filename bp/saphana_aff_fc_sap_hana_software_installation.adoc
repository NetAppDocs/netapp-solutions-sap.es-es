...
....
+
Si se utiliza el LVM de Linux, la configuración necesaria es diferente. En el siguiente ejemplo se muestra una configuración de varios hosts de 2+1 con SID=FC5.
...
NOTE: El directorio donde se preparó `global.ini` el archivo se almacena con la `storage_cfg` Opción CLI (`-- storage_cfg=/hana/shared`).
...
link:saphana_aff_fc_adding_additional_data_volume_partitions_for_sap_hana_single-host_systems.html["Siguiente: Añadir particiones de volúmenes de datos adicionales para sistemas SAP HANA de un solo host."]
