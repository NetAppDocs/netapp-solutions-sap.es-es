...
La siguiente tabla muestra la configuración de LUN de un sistema SAP HANA de un solo host.

|===
| Específico | LUN | Volumen lógico LVM 


| Datos | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| Registro | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| Compartido | FC5_shared | n.a. 
|===

NOTE: Con la configuración descrita, el  `/usr/sap/SID1` directorio en el que se almacena el directorio inicial por defecto del usuario SID1adm está en el disco local. En una configuración de recuperación de desastres con replicación basada en disco, NetApp recomienda crear un LUN adicional para el  `/usr/sap/SID1` directorio para que todos los sistemas de archivos estén en el almacenamiento central.


NOTE: Si se utiliza el servicio SAP Local Secure Store (LSS), el  `/lss/shared/SID` directorio, el directorio de instalación por defecto de LSS, se encuentra en el disco local. En una configuración de recuperación de desastres con replicación basada en disco, NetApp recomienda crear un LUN adicional para el  `/lss/shared/SID` directorio de modo que todos los sistemas de archivos se encuentren en el almacenamiento central.
