...
summary: A continuación se enumeran los requisitos para la instalación de software para SAP HANA. 
---
...
link:saphana-fas-nfs_sap_hana_data_volume_size.html["Anterior: Tamaño del volumen de datos de SAP HANA."]
...
. Uso de SAP `hdbclm` herramienta de instalación, inicie la instalación ejecutando el siguiente comando en uno de los hosts de trabajo. Utilice la `addhosts` opción para agregar el segundo trabajador (`sapcc-hana-tst-07`) y el host en espera (`sapcc-hana-tst-08`).
+
....
...
link:saphana-fas-nfs_adding_additional_data_volume_partitions.html["Siguiente: Adición de particiones de volumen de datos adicionales."]
