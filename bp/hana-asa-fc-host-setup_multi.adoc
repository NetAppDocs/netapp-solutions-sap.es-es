...
NOTE: Los sistemas de hosts múltiples SAP HANA requieren el  `/hana/shared` sistema de archivos y los  `/lss/shared` sistemas de archivos, en caso de que se utilice el servicio SAP Local Secure Store (LSS), conectados a todos los hosts de un sistema. Normalmente esto es un recurso compartido NFS proporcionado por un servidor NFS. Se recomienda usar un servidor NFS de alta disponibilidad, por ejemplo, como un sistema NetApp FAS o AFF. Otra opción es usar el servidor NFS integrado de un host LINUX para esto.
