...
Las instantáneas creadas por ONTAP Autonomous Ransomware Protection no se pueden usar para la recuperación posterior de la base de datos SAP HANA. Por eso, no se recomienda usarlas para los volúmenes de datos y registros de SAP HANA y deberías desactivar esta función en esos volúmenes. Usa la protección de datos basada en SnapCenter con instantáneas a prueba de manipulaciones para protegerte contra ransomware: link:../backup/hana-sc-generic-scope.html["Backup y recuperación de datos de SAP HANA con SnapCenter"]. ONTAP Autonomous Ransomware Protection puede ser útil para otros volúmenes como /hana/shared.

Es posible desactivar la activación automática de esta funcionalidad durante la creación de un volumen para un ONTAP Cluster, pero se recomienda mantener la configuración predeterminada y desactivarla solo en los volúmenes donde no se necesite esta funcionalidad.
...
image:saphana_aff_nfs_image13.png["Conexiones de bandeja SAS"]



=== Bandejas de discos NVMe (100 GbE)

Cada bandeja de discos NVMe NS224 está conectada con dos puertos 100GbE GbE por controladora, como se muestra en la figura siguiente. Los discos dentro de cada bandeja deben distribuirse de forma equitativa a ambas controladoras de la pareja de alta disponibilidad. ADPv2, como se describe en el capítulo sobre configuración de agregados, también se utiliza para la bandeja de discos NS224.

image:saphana_aff_nfs_image14a.png["Conexiones de bandeja de discos MV"]



== Configuración de agregados

En general, debe configurar dos agregados por controladora, independientemente de la bandeja de discos o la tecnología de unidades (SSD SAS o SSD NVMe) que se utilice.
...
image:saphana_aff_nfs_image15a.png["Configuración de agregado"]
...
image:saphana_aff_nfs_image16a.png["Hosts únicos de SAP HANA de configuración de volúmenes"]
...
NOTE: Si se utiliza el servicio SAP Local Secure Store (LSS), el  `/lss/shared/SID` directorio, el directorio de instalación predeterminado de LSS, se encuentra en el disco local. En una configuración de recuperación ante desastres con replicación basada en disco, NetApp recomienda crear un directorio dentro del  `SID_shared` volumen para el  `/lss/shared/SID` directorio, de modo que todos los sistemas de archivos se encuentren en el almacenamiento central.
...
image:saphana_aff_nfs_image17a.png["Configuración de volúmenes SAP HANA Varios hosts"]
...
NOTE: Si se utiliza el servicio SAP Local Secure Store (LSS), el  `/lss/shared/SID` directorio debe ser accesible en todos los hosts. NetApp recomienda crear un directorio dentro del  `SID_shared` volumen para el  `/lss/shared/SID` directorio.
...
| SID_shared | lss-shared | /lss/shared/SID | Montado en todos los hosts 
...
| Desactiva la protección contra ransomware, excepto en el volumen SID_shared | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled 
...
NOTE: Tenga en cuenta que la desactivación de id numéricos requiere la gestión de usuarios, como se describe en la sección link:hana-aff-nfs-install-prep-nfsv4.html["“Preparación de la instalación de SAP HANA para NFSv4”."]


NOTE: El ID de dominio NFSv4 debe establecerse con el mismo valor en todos los servidores Linux (`/etc/idmapd.conf`) y SVM, como se describe en la sección link:hana-aff-nfs-install-prep-nfsv4.html["“Preparación de la instalación de SAP HANA para NFSv4”."]
