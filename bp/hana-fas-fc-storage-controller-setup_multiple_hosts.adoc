...
El `/hana/shared` volumen debe ser accesible por todos los hosts HANA y, por tanto, se exporta utilizando NFS. Aunque no existen KPI de rendimiento específicos para el `/hana/shared` sistema de archivos, NetApp recomienda usar una conexión Ethernet de 10Gb.
...
NOTE: Si se utiliza el servicio SAP Local Secure Store (LSS), el  `/lss/shared/SID` directorio debe ser accesible en todos los hosts. NetApp recomienda crear un volumen para el  `/lss/shared/SID` directorio y montarlo en todos los hosts usando NFS.
...
Los LUNs de un grupo de volúmenes LVM deben almacenarse dentro de un agregado diferente y en un controlador diferente. La siguiente tabla muestra un ejemplo de dos LUNs por grupo de volúmenes para un sistema SAP HANA de host múltiple 2+1.
...
NOTE: El siguiente ejemplo establece estas opciones durante la creación del volumen
